...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    conv3_weights = tf.Variable(tf.truncated_normal([5, 5, 64, 128],stddev=0.1,seed=SEED))
    conv3_biases = tf.Variable(tf.constant(0.1, shape=[128]))
    
    fc1_weights = tf.Variable(  # fully connected, depth 512.
        tf.truncated_normal([int(IMG_PATCH_SIZE / 4 * IMG_PATCH_SIZE / 4 * 128), 512], stddev=0.1, seed=SEED))
