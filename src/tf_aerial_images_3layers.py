...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    def model(data, train=False):
        ...
        # Add a 50% dropout during training only. Dropout also scales activations such that no 
    ...
    #optimizer = tf.train.AdamOptimizer(learning_rate,0.0).minimize(loss, global_step=batch)
    init_op = tf.global_variables_initializer()

    correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(train_labels_node, 1))
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range (int(train_size / BATCH_SIZE)):
                    ...
                    if step % RECORDING_STEP == 0:
                        ...
                        summary_str, _, l, lr, predictions, acc = s.run([summary_op, optimizer, loss, learning_rate, train_prediction, accuracy],
                            feed_dict=feed_dict)
                        #summary_str = s.run(summary_op, feed_dict=feed_dict)
                        #loss, acc = s.run([cost, accuracy], feed_dict=feed_dict)
